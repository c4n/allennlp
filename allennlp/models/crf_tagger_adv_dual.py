...
@Model.register("crf_tagger_adv_dual")
class CrfTaggerAdvDual(Model):
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if tags is not None:
            ...
            dual_loss = torch.norm(avg_anchor-avg_positive,2,1)**2
            dual_loss = dual_loss.sum()# sum across the batches
            log_likelihood = self.crf(logits, tags, mask)
            adv_log_likelihood = self.crf(adv_logits, tags, adv_mask)

            output["loss"] = -log_likelihood -adv_log_likelihood*0.2 +dual_loss
