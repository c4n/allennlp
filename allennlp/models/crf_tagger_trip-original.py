    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.dropout:
            ...
            neg_encoded_text = self.dropout(neg_encoded_text)

        avg_anchor = torch.mean(encoded_text,1) 
        avg_positive = torch.mean(adv_encoded_text,1) 
        avg_negative = torch.mean(neg_encoded_text,1) 
...
#             neg_encoded_text = self._feedforward(neg_encoded_text)
        ...
        if tags is not None:
            ...
            triplet_loss = torch.norm(avg_anchor-avg_positive,2) - torch.norm(avg_anchor-avg_negative,2)
            ...
            output["loss"] = -log_likelihood -adv_log_likelihood*0.2 + triplet_loss*0.1
