...
def get_fields_to_compare_instance(
    input_instance: Instance, instance: Instance, input_field_to_attack: str
) -> JsonDict:
    """
    Gets a list of the fields that should be checked for equality after an attack is performed.
    ...
    fields_to_compare = {
        key: instance[key]
        for key in instance.fields
        if key not in input_instance.fields
        and key != input_field_to_attack
        and key != "metadata"
        and key != "output"
    }
...
def instance_has_changed(instance: Instance, fields_to_compare: JsonDict):
    ...
    if any(
        instance[field].labels != fields_to_compare[field].labels for field in fields_to_compare
    ):
        return True
    return False
